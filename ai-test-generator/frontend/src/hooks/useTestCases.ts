...
export const useTestCases = (): UseTestCasesReturn => {
  ...
  const saveTestCase = useCallback(
    async (testCase: Partial<TestCase>): Promise<TestCase> => {
      setError(null);
      ...
      try {
        ...
        setTestCases((prevCases) => {
          if (testCase.id) {
            ...
            return prevCases.map((tc) =>
              tc.id === testCase.id ? savedTestCase : tc
            );
          } else {
            // Add new test case
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to save test case";
        setError(errorMessage);
        throw err;
      }
    },
  ...
  const deleteTestCase = useCallback(async (id: string): Promise<void> => {
    ...
    try {
      ...
      // Update local state
      setTestCases((prevCases) =>
        prevCases.filter((tc) => tc.id !== parseInt(id, 10))
      );
    } catch (err) {
      const errorMessage =
  ...
  const findSimilarTestCases = useCallback(
    async (criteria: string): Promise<TestCase[]> => {
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to find similar test cases";
        setError(errorMessage);
        throw err;
      }
    },
